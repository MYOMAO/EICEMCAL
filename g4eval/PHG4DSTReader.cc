...
int PHG4DSTReader::Init(PHCompositeNode *)
{
	...
	//Initial Output Shower Energy Tree //

	fout = new TFile("ShowerInfo.root","RECREATE");
	fout->cd();
	ShowerInfo = new TTree("ShowerInfo","ShowerInfo");


	ShowerInfo->Branch("evtid",&evtid,"evtid/I");
	ShowerInfo->Branch("hitid",&hitid,"hitid/I");
	ShowerInfo->Branch("trkid",&trkid,"trkid/I");
	ShowerInfo->Branch("showerid",&showerid,"showerid/I");
	ShowerInfo->Branch("layer",&layer,"layer/I");

	ShowerInfo->Branch("X1",&X1,"X1/F");
	ShowerInfo->Branch("X2",&X2,"X2/F");
	ShowerInfo->Branch("Y1",&Y1,"Y1/F");
	ShowerInfo->Branch("Y2",&Y2,"Y2/F");
	ShowerInfo->Branch("Z1",&Z1,"Z1/F");
	ShowerInfo->Branch("Z2",&Z2,"Z2/F");
	ShowerInfo->Branch("T1",&T1,"T1/F");
	ShowerInfo->Branch("T2",&T2,"T2/F");



	ShowerInfo->Branch("GPX1",&GPX1,"GPX1/F");
	ShowerInfo->Branch("GPY1",&GPY1,"GPY1/F");
	ShowerInfo->Branch("GPZ1",&GPZ1,"GPZ1/F");


	ShowerInfo->Branch("GPX2",&GPX2,"GPX2/F");
	ShowerInfo->Branch("GPY2",&GPY2,"GPY2/F");
	ShowerInfo->Branch("GPZ2",&GPZ2,"GPZ2/F");


	ShowerInfo->Branch("GE1",&GE1,"GE1/F");
	ShowerInfo->Branch("GE2",&GE2,"GE2/F");

	ShowerInfo->Branch("NumOfE",&NumOfE,"NumOfE/I");


	ShowerInfo->Branch("BHBLedep",&BHBLedep,"BHBLedep/F"); 
	ShowerInfo->Branch("BHFLedep",&BHFLedep,"BHFLedep/F");
	ShowerInfo->Branch("FEMCALedep",&FEMCALedep,"FEMCALedep/F");
	ShowerInfo->Branch("FEMCALLYDep",&FEMCALLYDep,"FEMCALLYDep/F");
	ShowerInfo->Branch("FEMCALEIonDep",&FEMCALEIonDep,"FEMCALEIonDep/F");
	ShowerInfo->Branch("AbsorberDep",&AbsorberDep,"AbsorberDep/F");
	ShowerInfo->Branch("ERECO",&ERECO,"ERECO/F");
	ShowerInfo->Branch("EOut",&EOut,"EOut/F");

	//  ShowerInfo->Branch("NClusters",&NClusters,"NClusters/I");

	finDebug = new TFile("Debug.root","RECREATE");
	finDebug->cd();
	DebugTree = new TTree("DebugTree","DebugTree");
	DebugTree->Branch("LayerID",&LayerID,"LayerID/I");
	DebugTree->Branch("BlackHole",&BlackHole,"BlackHole/I");
	DebugTree->Branch("IsActive",&IsActive,"IsActive/I");
	DebugTree->Branch("EMCAL",&EMCAL,"EMCAL/I");
	DebugTree->Branch("BHLAYER",&BHLAYER,"BHLAYER/I");
...
int PHG4DSTReader::process_event(PHCompositeNode *topNode)
{
	...
	for (records_t::iterator it = _records.begin(); it != _records.end(); ++it)
	{
		...
		if (rec._type == record::typ_hit)
		{
			...
			else
			{
				...
				//std::cout << "Been here bro BEFORE INSIDE" << std::endl;
				...
				//	std::cout << "Been here bro AFTER INSIDE" << std::endl;
			}  // if (!hits)
		}    //      if (rec._type == record::typ_hit)
	...
	//  cout << "PHG4Hit: " << "  GE1: " << GE1 << "  GPX1 = " << GPX1 <<   " GPY1 = " << GPY1 <<  " GPZ1 = " << GPZ1 << endl;
	//  cout << "PHG4Hit: " << "  GE2: " << GE2 << " GPX2 = " << GPX2 <<   " GPY2 = " << GPY2 <<  " GPZ2 = " << GPZ2 << endl;

	// cout << "NumOfE = " << NumOfE << endl;
...
int PHG4DSTReader::End(PHCompositeNode * /*topNode*/)
{
	...
	std::cout << "Now Destroy Shits" << std::endl;
	fout->cd();	
	ShowerInfo->Write();
	fout->Close();
