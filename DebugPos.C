...
void DebugPos(){
	...
	TString infile = "PosDebug/G4EICDetector.root_g4femc_eval.root";
	...
	TVector3 * Truth = new TVector3;
	Truth->SetXYZ(0,0,1);
	double CosAngle;

	TVector3 * Clus = new TVector3;

	int NEntries = ntp_cluster->GetEntries();

	std::ofstream DebugBro2("DatPos/DebugEvt2.dat",ios::app);
	...
	//	DebugBro2  << "Index = " << Index << "    FUCKIN TotalEnergy = " << TotalEnergy << "   Energy = " << Energy << "   Ratio = " << Ratio << endl;
		...
		CosAngle = Clus->Unit().Dot(Truth->Unit());
	...
	AngleDis->Draw();
	c->SaveAs("PosDebug/AngleDis.png");
