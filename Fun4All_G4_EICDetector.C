...
int Fun4All_G4_EICDetector(
    const int nEvents = 1,
	double BeamEnergy = 5.0,
	double EtaInput = 100.0,
	int Index = 0,
...
//	 se->Verbosity(0); // uncomment for batch production running with minimal output messages
 ...
 // int SeedSuck = PHRandomSeed();
  
  int SeedSuck = 267910556;
  
  std::ofstream DebugBro("DatFiles/DebugEvt.dat",ios::app);

  
  cout << "--------------------------------------------- Now Random Seed ---------------------------------------------" << endl;
  cout << "SeedSuck = " << SeedSuck << endl;
  DebugBro << "Index = " << Index << "    SeedSuck = "  << SeedSuck << endl;
  cout << "--------------------------------------------- DONE Random Seed ---------------------------------------------" << endl;
...
// return 0;
 ...
 //  rc->set_IntFlag("RANDOMSEED",SeedSuck);
  //-----------------
  // Event generation
  //-----------------
  cout << "Pass 2.0" << endl;
...
*/
  else if (runsartre)
  {
    ...
    pTrig->SetEtaHighLow(0, 1);  // central arm
    pTrig->PrintConfig();
...
//  gen->add_particles("pi0", 1);  // mu+,e+,proton,pi+,Upsilon
//  gen->add_particles("gamma",1);  
cout << "Pass 2.3" << endl;
  ...
  //gen->add_particles("pi+",100); // 100 pion option
    ...
    else
    {
      gen->set_vertex_distribution_function(PHG4SimpleEventGenerator::Uniform,
                                            ...
                                            PHG4SimpleEventGenerator::Uniform);
   //   gen->set_vertex_distribution_mean(75.0, 75.0, 100.0);
   // gen->set_vertex_distribution_mean(25.0, 25.0, 100.0);
	...
	double EtaU = EtaInput;
	double EtaL = EtaInput;
 double eMass = 0.000511; 	
	//double eMass = 0.1057; 	
//	double eMass = 0.13957018;
//	double eMass = 0.1349768;
//       double eMass = 0;
  cout << "Pass 3" << endl;

	double ptU = sqrt(BeamEnergy * BeamEnergy - eMass * eMass)/TMath::CosH(EtaL);
	double ptL = sqrt(BeamEnergy * BeamEnergy - eMass * eMass)/TMath::CosH(EtaU); 
	double pL =  sqrt(BeamEnergy * BeamEnergy - eMass * eMass);
	double pU =  sqrt(BeamEnergy * BeamEnergy - eMass * eMass);

//	double ptU = BeamEnergy/TMath::CosH(EtaL);
//	double ptL = BeamEnergy/TMath::CosH(EtaU);

	/*
	cout << "------------------------------------------------------------" << endl;
	cout << "BeamEnergy = " << BeamEnergy << "   pt = " << ptL << endl;
	cout << "------------------------------------------------------------" << endl;
	*/	

	gen->set_eta_range(EtaL,EtaU);
//   gen->set_phi_range(-1.0 * TMath::Pi(), 1.0 * TMath::Pi());
 ...
 //   gen->set_pt_range(5, 5.0);
	...
	//gen->set_pt_range(ptL,ptU);
	gen->set_p_range(pL,pU);
	gen->Embed(1);
    gen->Verbosity(0);
