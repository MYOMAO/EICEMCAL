...
void PlotEDepDepthLoop(){
	...
	double ELeakErr;
	const int NFile = 10;
	double NLayers[NFile] = {1,2,5,10,20,50,66,100,200,500};


	TH1D * EnergyLeak = new TH1D("EnergyLeak","",500,0.9,1000);

	EnergyLeak->GetXaxis()->SetTitle("Number of Layers");
	...
	//	Energy = Energymin + Energystep * q;
		...
		infile	= Form("DepthScan/G4EICDetector.root_DSTReader_%d.root",q);
		...
		c->SaveAs("ELeakAnaDepth/SingleEvent.png");
...
*/
		...
		TH1D * FEMALEdepTest = new TH1D("FEMALEdepTest","",5000,0,5000);
		TH1D * BHForwardEdepTest = new TH1D("BHForwardEdepTest","",100,0,5000);
		TH1D * BHBackwardEdepTest = new TH1D("BHBackwardEdepTest","",5000,0,5000);



		TH1D * TotalFEMCHis = new TH1D("TotalFEMCHis","",25,1600,2000);
		TH1D * TotalForwardBHHis = new TH1D("TotalForwardBHHis","",200,0,5000);
		TH1D * TotalBackwardBHHis = new TH1D("TotalBackwardBHHis","",25,0,20);
...
*/
			...
			cout << "TotalEnergyForward = " << TotalEnergyForward << endl;
		...
		c2->SaveAs(Form("ELeakAnaDepth/LeakSumDisForward_%d.png",q));

		TotalBackwardBHHis->Draw();
		c2->SaveAs(Form("ELeakAnaDepth/LeakSumDisBackward_%d.png",q));

		TotalFEMCHis->Draw();
		c2->SaveAs(Form("ELeakAnaDepth/LeakSumDisFEMCAL_%d.png",q));


		ELeakMean = TotalForwardBHHis->GetMean();
		ELeakErr = TotalForwardBHHis->GetRMS();
	
		XBin = EnergyLeak->GetXaxis()->FindBin(NLayers[q]);
		

		EnergyLeak->SetBinContent(XBin,ELeakMean);
		EnergyLeak->SetBinError(XBin,ELeakMean/sqrt(1000));
...
*/
	lat->DrawLatex(0.15,0.75,"Incident Electron Energy = 5 GeV");

	c3->SaveAs("ELeakAnaDepth/All/EnergyLeak.png");
