...
//	g4hitntuple = new TNtuple("hitntup", "G4Hits", "x0:y0:z0:x1:y1:z1:edep");
	g4hitntuple = new TNtuple("ShowerInfo", "G4Hits", "evtid:layer:X1:X2:Y1:Y2:Z1:Z2:FEMCALedep:BHBLedep:BHFLedep");
...
int CaloAna::process_event(PHCompositeNode* topNode)
{
	...
	//cout << "topNode Name = " << topNode->getName();
...
//	cout << "In process Calo" << endl;

	// loop over the G4Hits in FEMCAL
	nodename.str("");
	nodename << "G4HIT_" << detector1;

//	cout << "INJection string = " << nodename.str().c_str() << endl;
	...
	//	cout << "   sucked in "  << "hit_range = " << hits->size() << endl;

		for (PHG4HitContainer::ConstIterator hit_iter = hit_range.first; hit_iter != hit_range.second; hit_iter++)
		{
			//   cout << "sucked in 2 "  << endl;

			// the pointer to the G4Hit is hit_iter->second
			g4hitntuple->Fill(evtid,
					-1,
...
//	cout << "Now BH" << endl;
	//BlackHole//
	ostringstream nodenameBH;

	nodenameBH.str("");
	nodenameBH << "G4HIT_" << detector2;

	PHG4HitContainer* hitsBH = findNode::getClass<PHG4HitContainer>(topNode, nodenameBH.str().c_str());

	if (hits)
	{
		PHG4HitContainer::ConstRange hit_range = hitsBH->getHits();

//		cout << "BH_1 hit_range = " << hitsBH->size() << endl;
...
//	cout << "Now Forward Plus BH" << endl;
	//BlackHole//
	ostringstream nodenameBHFP;

	nodenameBHFP.str("");
	nodenameBHFP << "G4HIT_" << detector3;

	PHG4HitContainer* hitsBHFP = findNode::getClass<PHG4HitContainer>(topNode, nodenameBHFP.str().c_str());

	if (hits)
	{
		PHG4HitContainer::ConstRange hit_range = hitsBHFP->getHits();

	//	cout << "BH_FORWARD_PLUS hit_range = " << hitsBHFP->size() << endl;
...
//	cout << "Now Forward Neg BH" << endl;
	//BlackHole//
	ostringstream nodenameBHFN;

	nodenameBHFN.str("");
	nodenameBHFN << "G4HIT_" << detector4;

	PHG4HitContainer* hitsBHFN = findNode::getClass<PHG4HitContainer>(topNode, nodenameBHFN.str().c_str());

	if (hits)
	{
		PHG4HitContainer::ConstRange hit_range = hitsBHFN->getHits();

//		cout << "BH_FORWARD_NEG hit_range = " << hitsBHFN->size() << endl;
...
int CaloAna::process_towers(PHCompositeNode* topNode)
{
	...
	// loop over the G4Hits
	nodename.str("");
	nodename << "TOWER_CALIB_" << detector1;
	geonodename.str("");
	geonodename << "TOWERGEOM_" << detector1;
