...
void PlotEDep(int Energy, int BeamOpt, int AbsOpt){
	...
	if(AbsOpt == 1) AbsMaterial = "WCu";
	if(AbsOpt == 2) AbsMaterial = "Pb";
	...
	TString infile = Form("ShowerInfo_%d_%s_%s.root",Energy,BeamName.Data(),AbsMaterial.Data());
	...
	TH1D * EDepTransWAbsScale = new TH1D("EDepTransWAbsScale","",100,0,10);
	EDepTransWAbsScale->GetXaxis()->SetTitle("WCu EMCAL Radius (cm)");
	EDepTransWAbsScale->GetYaxis()->SetTitle("dE/dR (GeV/cm)");
	EDepTransWAbsScale->SetTitle("Incident e^{-} Transverse Energy Deposition");
	EDepTransWAbsScale->GetXaxis()->CenterTitle();
	EDepTransWAbsScale->GetYaxis()->CenterTitle();
	EDepTransWAbsScale->GetYaxis()->SetTitleOffset(1.4);
	EDepTransWAbsScale->SetLineColor(kBlack);
	

	double AbsWidth = EDepTransWAbsScale->GetBinWidth(1);
...
//		RPosCorrFT = sqrt(XPosCorrFT * XPosCorrFT + YPosCorrFT * YPosCorrFT);
		...
		if(layer == -1){
			...
			EDepTransWAbsScale->Fill(sqrt(XPosCorr * XPosCorr + YPosCorr * YPosCorr),FEMCALedep/AbsWidth);	
	...
	//	EDepLongWFT->Draw("histSAME");
		...
		leg->AddEntry(EDepLongW,Form("%s-Shashlik (Mine, Position Corrected)",AbsMaterial.Data()),"l");	
//		leg->AddEntry(EDepLongWFT,"W-Shashlik (Mine, Position Corrected + Angle Rotated)","l");	
	
		leg->Draw("SAME");
		c->SaveAs(Form("NewStudy/%s/%s/Longitudinal/EDepthLongStudy1GeV.png",AbsMaterial.Data(),BeamName.Data()));
...
//		EDepTransWCumCorrFT->Draw("histSAME");	
		...
		lat->DrawLatex(0.30,0.65,Form("Scintillator: %.1f mm Polystyrene",ScinThick*10));
...
//		legTrans->AddEntry(EDepTransWCum,"W-Shashlik (Mine, No Position Correction)","l");
		legLong->AddEntry(EDepLongWCum,Form("%s-Shashlik (Mine, Position Corrected)",AbsMaterial.Data()),"l");
//		legTrans->AddEntry(EDepTransWCumCorrFT,"W-Shashlik (Mine, Position Corrected + Angle Rotated)","l");

		legLong->Draw("SAME");

		c->SaveAs(Form("NewStudy/%s/%s/Longitudinal/EDepthLingStudy1GeVCum.png",AbsMaterial.Data(),BeamName.Data()));


		EvsZ->Draw();
		c->SaveAs(Form("NewStudy/%s/%s/Debug/EvsZ.png",AbsMaterial.Data(),BeamName.Data()));


		//Transverse Profile
		
		EDepTransWAbsScale->Scale(1.0/NEvents);

		EDepTransWAbsScale->Draw("hist");
		lat->DrawLatex(0.30,0.80,Form("Incident %s Eneregy = 1 GeV",BeamName.Data()));	
		lat->DrawLatex(0.30,0.75,Form("Radiation Length of 80W/20Cu: X0 = %.1f mm",WRadLength*10));				
		lat->DrawLatex(0.30,0.70,"Sampling Thickness: 1/4 X0");	
		lat->DrawLatex(0.30,0.65,"Total RadLength: 20 X0");
		lat->DrawLatex(0.30,0.60,Form("Scintillator: %.1f mm Polystyrene",ScinThick*10));
	
		c->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepTransWAbsScale.png",AbsMaterial.Data(),BeamName.Data()));
		...
		lat->DrawLatex(0.30,0.65,"Total RadLength: 20 X0");
		lat->DrawLatex(0.30,0.60,Form("Scintillator: %.1f mm Polystyrene",ScinThick*10));
		cLog->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepTransWAbsLogScale.png",AbsMaterial.Data(),BeamName.Data()));
...
//		legTrans->AddEntry(EDepTransWCum,"W-Shashlik (Mine, No Position Correction)","l");
		legTrans->AddEntry(EDepTransWCumCorr,Form("%s-Shashlik (Mine, Position Corrected)",AbsMaterial.Data()),"l");
//		legTrans->AddEntry(EDepTransWCumCorrFT,"W-Shashlik (Mine, Position Corrected + Angle Rotated)","l");
		...
		c->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepthTransStudy1GeV.png",AbsMaterial.Data(),BeamName.Data()));

		EDepTransWCumCorr->Draw("hist");
		c->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepTransWCumCorr.png",AbsMaterial.Data(),BeamName.Data()));
		...
		c->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepXY.png",AbsMaterial.Data(),BeamName.Data()));
		...
		lat->DrawLatex(0.30,0.70,"Total RadLength: 20 X0");
		lat->DrawLatex(0.30,0.65,Form("Scintillator: %.1f mm Polystyrene",ScinThick*10));
		c3->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepXYCorr.png",AbsMaterial.Data(),BeamName.Data()));
	...
	//	EDepXYCorrFT->Draw("COLZ");
		...
		lat->DrawLatex(0.30,0.70,"Total RadLength: 20 X0");
		lat->DrawLatex(0.30,0.65,Form("Scintillator: %.1f mm Polystyrene",ScinThick*10));
		c3->SaveAs(Form("NewStudy/%s/%s/Transverse/EDepXYCorrFT.png",AbsMaterial.Data(),BeamName.Data()));
	...
	ETotalHis->Draw();
	c->SaveAs(Form("NewStudy/%s/%s/Debug/EnergyConservationCheck.png",AbsMaterial.Data(),BeamName.Data()));
	...
	if(Energy == 10){
		...
		leg->Draw("SAME");
		c->SaveAs(Form("NewStudy/%s/%s/Comparison/EDepthLongStudy10GeV.png",AbsMaterial.Data(),BeamName.Data()));
	}
