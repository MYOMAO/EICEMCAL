...
void DebugEvtAnaNew(int Energy, int BeamOpt){
	...
	EnergyHisBefore->GetXaxis()->SetTitle("Total Energy Before Addition (GeV)");
	...
	for(int i = 0; i < 3; i++){
		...
		TotalEnergyAfter = EOut + BeamMass;
	...
	for(int i = 0; i < NEntries; i++){
		...
		evtidPre = evtid;
			...
			TotalEnergyBefore = BHFLedep + FEMCALedep + BHBLedep + TotalEnergyBefore;
