...
void FEMC_Cells(int verbosity = 0)
{
  ...
  cout << "FUCK CALOANAN FUCKIN LATE" << endl;
  ...
  ca->Detector1("FEMC");
  ca->Detector2("BH_1");
  ca->Detector3("BH_FORWARD_PLUS");
  ca->Detector4("BH_FORWARD_NEG");

  se->registerSubsystem(ca);

  cout << "FUCK DONE BRO" << endl;
...
// tower_FEMC->GeometryTableFile();
  ...
  TowerCalibration2->set_calib_const_GeV_ADC(1.0 / 0.249 * CorrFactor);  // sampling fraction = 0.249 for e-
...
void FEMC_Clusters(int verbosity = 0)
{
  ...
  if (G4FEMC::Femc_clusterizer == G4FEMC::kFemcTemplateClusterizer)
  {
	...
	//ClusterBuilder->SetHisTogram(HisTowerXIndex,HisTowerYIndex);
