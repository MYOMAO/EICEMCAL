...
void PlotEDepPb(int Energy){
	...
	TString infile = Form("ShowerInfo_%d_Pb.root",Energy);
	...
	TH1D * ETotalHis = new TH1D("ETotalHis","",100,0,1);
	ETotalHis->GetXaxis()->SetTitle("Total Energy Fraction");
	...
	EDepLongW->GetYaxis()->SetTitle("Transverse Energy Profile (% of E_{inc})");
	...
	*/
	...
	double VtxX = MinDepthXY * cos(phi);
	double VtxY = MinDepthXY * sin(phi);
	...
	for(int i = 0; i < NEntries; i++){
		...
		RPos = (sqrt(X1 * X1 + Y1 * Y1) - MinDepthXY); 
...
//		RPosCorrFT = sqrt(XPosCorrFT * XPosCorrFT + YPosCorrFT * YPosCorrFT);
		...
		TotalEnergyCounted = FEMCALedep + BHBLedep + BHFLedep;
		TotalEnergyCountedCum = FEMCALedep + BHBLedep + BHFLedep;
	}
	...
	if(Energy == 1){
		lat->DrawLatex(0.30,0.80,"Incident Electron Eneregy = 1 GeV");	
		...
		leg->Draw("SAME");
		c->SaveAs("NewStudyPb/Longitudinal/EDepthLongStudy1GeV.png");
		...
		EDepTransWCumCorr->SetMinimum(0);
...
//		legTrans->AddEntry(EDepTransWCum,"W-Shashlik (Mine, No Position Correction)","l");
		legTrans->AddEntry(EDepTransWCumCorr,"Pb-Shashlik (Mine, Position Corrected)","l");
		legTrans->AddEntry(EDepTransWCumCorrFT,"Pb-Shashlik (Mine, Position Corrected + Angle Rotated)","l");
		...
		c->SaveAs("NewStudyPb/Transverse/EDepthTransStudy1GeV.png");

		EDepTransWCumCorr->Draw("hist");
		c->SaveAs("NewStudyPb/Transverse/EDepTransWCumCorr.png");
		...
		c->SaveAs("NewStudyPb/Transverse/EDepXY.png");
		...
		lat->DrawLatex(0.30,0.70,"Total RadLength: 20 X0");
		lat->DrawLatex(0.30,0.65,"Scintillator: 1.4 mm Polystyrene");
		c3->SaveAs("NewStudyPb/Transverse/EDepXYCorr.png");
		...
		lat->DrawLatex(0.30,0.70,"Total RadLength: 20 X0");
		lat->DrawLatex(0.30,0.65,"Scintillator: 1.4 mm Polystyrene");
		c3->SaveAs("NewStudyPb/Transverse/EDepXYCorrFT.png");
	...
	ETotalHis->Draw();
	c->SaveAs("NewStudy/Debug/ETotalHis.png");
	...
	for(int i = 0; i < NPoints1GeVTrans; i++){
	...
	lat->DrawLatex(0.20,0.75,"Inciden Electron Beam Energy = 1 GeV");
	...
	lat->DrawLatex(0.20,0.75,"Inciden Electron Beam Energy = 10 GeV");
	lat->DrawLatex(0.20,0.70,Form("4X0 Cumulative Energy Deposition = %.1f %%",PercentDepth ));
