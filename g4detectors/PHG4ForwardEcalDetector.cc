  ...
  , m_TowerLogicNamePrefix("hEcalTower")
{
  for (int i = 0; i < 3; i++)
  {
	...
	cout << "ROCK BRO lol:   " << " i = " << i << "m_TowerDz[i] = "  << m_TowerDz[i] << endl;
  ...
  m_RMax[1] = 2460 * mm;
   ...
   cout << "m_dZ = " << m_dZ << "   m_RMin = " <<   m_RMin[0]  << "   m_RMax[0] = " << m_RMax[0] << endl;
//  float factor = 10;

  ifstream in;
  in.open("parameters/Par.dat");
  float tem1;
  float tem2;

  cout << "m_TowerDz 1 = " << m_TowerDz[2] << endl;
...
//  m_RMin[1] = 0.1 * mm;
  ...
  cout << "m_AbsorberActiveFlag = " << m_AbsorberActiveFlag << endl;

	
   m_AbsorberActiveFlag = 1;
...
//  m_TowerDx[2] = m_TowerDx[2] * factor;
//  m_TowerDy[2] = m_TowerDy[2] *factor;
...
PHG4ForwardEcalDetector::ConstructTower(int type)
{
  ...
  G4Material* material_air = G4Material::GetMaterial("G4_Galactic");
  ...
  cout << "building type " << type << " towers" << "   m_TowerDz[type]  = " << m_TowerDz[type]  << endl;
...
//  G4Material* material_air = G4Material::GetMaterial("G4_Galactic");
  ...
  cout << "m_TowerDz Before = " << m_TowerDz[2] << endl;

  float TotalRadLength = 18;
  float RadThickFrac = 0.25;	

  float scalar = 1;
  float WRadLength = 5.6;
  float LayerRadLength = WRadLength * RadThickFrac;
  float AbSciRatio = WRadLength/1.5;

//  G4double thickness_absorber = 1.5 ;      // W 1.5mm absorber  W: 1X0 = 3.5 mm
 // G4double thickness_scintillator = 4.0;

//    G4double thickness_absorber = 1.47321;      // W 1.5mm absorber  W: 1X0 = 3.5 mm
//  G4double thickness_scintillator = LayerRadLength/(scalar * AbSciRatio);  // W 4mm scintillator
//  G4double thickness_scintillator = 3.92857;


 
   G4double thickness_absorber = 1.5 ;      // W 1.5mm absorber  W: 1X0 = 3.5 mm
   G4double thickness_scintillator = 4.0;
  // G4double thickness_scintillator = 1.0;

   G4int nlayers = TotalRadLength/RadThickFrac;


//  m_TowerDz[2] = nlayers * (thickness_absorber + thickness_scintillator + 0.1);
//  G4double thickness_layer = m_TowerDz[2] / (float) nlayers;
//  thickness_layer = 5.5;
//  nlayers = 66;
  //cout << "m_TowerDx After = " << m_TowerDx[2]  << "     m_TowerDy After = " << m_TowerDy[2]  << "     m_TowerDz After = " << m_TowerDz[2] << endl;

  m_TowerDz[2] = nlayers * (thickness_absorber + thickness_scintillator + 0.00);
  G4double thickness_layer = m_TowerDz[2] / (float) nlayers;

  cout << "SHITS : " << "   m_TowerDz[2] = " <<  m_TowerDz[2] << "  thickness_layer =  " << thickness_layer  <<  "   thickness_absorber = " << thickness_absorber << "  thickness_scintillator =  " << thickness_scintillator << endl;

  cout << "SUCK Dimension Print Out: " << "    m_TowerDx[2] = "  <<  m_TowerDx[2] << "  m_TowerDy[2] = " << m_TowerDy[2] << "   m_TowerDz[2] = " << m_TowerDz[2]  << endl;
...
//  G4double thickness_scintillator = thickness_layer * (4.0/5.6);  // 4mm scintillator
  ...
  cout << "SUCK:: thickness_layer = " << thickness_layer << endl;
...
//  G4Material* material_absorber = G4Material::GetMaterial("G4_W");
  ...
  cout << "Option1:: Fuckin zpos_i = " << zpos_i/10.0 << "   Net Z POS = " << zpos_i/10.0 + m_PlaceZ - 100 << endl;
  cout << "Option2:: Fuckin zpos_i = " << zpos_i/10.0 << "   Net Z POS = " << zpos_i/10.0 + 310 - 100 << endl;
...
PHG4ForwardEcalDetector::ConstructTowerType3_4_5_6(int type)
{
  ...
  cout << "Fucks Here bro::: " << "   num_fibers_x = " << num_fibers_x << "  num_fibers_y = " << num_fibers_y << endl;
...
//  istream_mapping.open(m_Params->get_string_param("mapping_file"));
  ...
  // istream_mapping.open("towerMap_FEMC_v007.txt");
  istream_mapping.open("towerMap_FEMC_latest.txt");
  ...
  cout << "FUCKIN FILE = " << m_Params->get_string_param("mapping_file") << endl;
  ...
  cout << "m_TowerDz 7 = " << m_TowerDz[2] << endl;
