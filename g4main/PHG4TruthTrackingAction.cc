...
//  cout << "OK FICK FUCK TRACK BRO ZZ DONE" << endl;
  ...
  double etot =( ke + m)/GeV;
  G4ThreeVector pdir = track->GetVertexMomentumDirection();
  pdir *= ptot;

  int nElectrons;

  //cout << "track->GetParentID() = " << track->GetParentID() << "   pdgid = " << pdgid << endl;

  if(track->GetParentID() == 0 && pdgid == 22){
   	  iGamma = 0;
 //  cout << "iGammaBefore = " << iGamma << "   GammaPX1 = " << GammaPX1 << endl;

	  GammaPX[iGamma] = pdir[0];
	  GammaPY[iGamma] = pdir[1];
	  GammaPZ[iGamma] = pdir[2];
	  GammaE[iGamma] = etot;
	//  cout << "iGamma = " << iGamma << "   Px = " << pdir[0] << "   Py = " << pdir[1] << "  Pz =" <<  pdir[2]  << "  etot  = " << etot  << endl;
	//  cout << "iGamma = " << iGamma << "   x = " << x << "   y = " << y << "  z =" <<  z << endl;


	//  cout << "In = " << iGamma << "   GammaPX[iGamma] = " << GammaPX[iGamma] << "   GammaPY[iGamma] = " << GammaPY[iGamma] << "   GammaPZ[iGamma] = " << GammaPZ[iGamma] << endl;
 ...
 // if(track->GetParentID() == 111 || pdgid == 111) cout  << "pdgid = " << pdgid << "    ke = " << ke/ GeV << "   etot = " << etot / GeV<< endl;
...
//  double r = sqrt((x - PVx) * (x - PVx) + (y - PVy) * (y - PVy) + (z - PVz) * (z - PVz));
//  int sign = 1;
//  if (z - PVz != 0) sign = (z - PVz)/abs(z - PVz);	
  std::ofstream shower("particleshower.dat",ios::app);
  if(abs(pdgid == 11) || abs(pdgid == 22)) TotalENERGY = TotalENERGY + etot;

  shower << "Cumulative Energy = " << TotalENERGY  << "  pdgid = " << pdgid  << "   etot = " << etot << "  px = " << pdir[0] << "   py = " << pdir[1]  << "   pz = " << pdir[2] << "   x = " << x <<  " y  = " << y << "   z = " << z  << endl;
