...
PHG4Hit::CopyFrom(const PHObject *phobj)
{
  ...
  for (int i =0; i<2; i++)
	...
	//  cout << "edep = "  << g4hit->get_edep() << endl;
	set_hit_id(g4hit->get_hit_id());
	set_shower_id(g4hit->get_shower_id());
	set_trkid(g4hit->get_trkid());

	// cout << "layer = " << g4hit->get_layer() << endl;
	//int detid =  g4hit->get_detid();
	...
	hitid = g4hit->get_hit_id();
	trkid = g4hit->get_trkid();
	showerid = g4hit->get_shower_id();
	layer = g4hit->get_layer();

	if(layer != 1 && layer != -1) cout << "Fuckin Layer Missing Bro --- Your life sucks Layer NOT COUNTED:  " << layer << endl;
	...
	FEMCALedep = 0;
	EOut = 0;
//	cout << "Layer id = " << layer << endl;

//	cout << "evtid = " << evtid << endl;

	if(Z1 > 0 && Z2 < 0 ) cout << "WTH" << endl;
	if(Z1 < 0 && Z2 > 0 ) cout << "WTH2" << endl;

	if(layer == 1 && Z1 > 0 && Z2 > 0){
	
		BHBLedep = g4hit->get_edep();

	}
	if(layer == 1 && Z1 < 0 && Z2 < 0){

		BHFLedep = g4hit->get_edep();

	}
	if(layer == -1){

		FEMCALedep = g4hit->get_edep();
//		FEMCALedep = g4hit->get_light_yield();
		
		FEMCALLYDep = g4hit->get_light_yield();
		FEMCALEIonDep = g4hit->get_eion();

	}
	if(layer == 0){

		AbsorberDep = g4hit->get_edep();

	}
	//cout << "ERECOGlobal2 = " << ERECOGlobal << endl;
	EOut = OutSideE; 


	ERECO = ERECOGlobal;
//	NClusters = NClustersGlobal;
    ShowerInfo->Fill();
