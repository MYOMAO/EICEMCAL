...
PHG4ForwardEcalDetector::ConstructTowerType2()
{
  ...
  float TotalRadLength = 20;
  float RadThickFrac = 0.25;	

  float scalar = 1;
  float WRadLength = 3.5;
  float LayerRadLength = WRadLength * RadThickFrac;
  float AbSciRatio = WRadLength/1.4;

  G4double thickness_absorber = LayerRadLength/scalar ;      // W 1.5mm absorber  W: 1X0 = 3.5 mm
//  G4double thickness_scintillator = LayerRadLength/(scalar * AbSciRatio);  // W 4mm scintillator
  G4double thickness_scintillator = 1.4;
